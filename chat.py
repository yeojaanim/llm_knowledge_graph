...
def graphcategory(parsing_output):
    ...
    categorized_data = [[item[0].strip(), item[1].strip(), item[2].strip()] for item in data]
...
def auto_knowledge_graph(context):
    ...
    pasring_output = parse_output(llm_output)
    categorized_data = graphcategory(pasring_output)
    return categorized_data
